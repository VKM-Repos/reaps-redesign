...
export default function TemplateCard({ template }: { template: any }) {
  return (
    <div
      className={`relative w-full md:max-w-[17.5rem] flex flex-col gap-8 justify-left p-4 rounded-2xl hover:bg-[#14155E14] hover:border-none border border-[#0C0C0F29] card-shadow`}
    >
      <div className="my-3 flex justify-between items-center text-[#868687]">
        ...
        <a href={template?.file_path} download>
          {" "}
          <Download />
        </a>
      </div>
      <div className="text-black">
        <p className="text-sm font-bold w-[200px] max-w-[200px] min-w-[200px]">
          {template?.title}
        </p>
      </div>
    </div>
