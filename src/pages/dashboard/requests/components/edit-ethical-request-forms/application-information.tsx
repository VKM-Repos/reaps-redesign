...
export default function ApplicationInformation({
  ...
  // Merge default values from props and store
  const defaultValues: QuestionsSchema = {
    are_you_investigator_or_local_principal_investigator: Boolean(
      data.ethical_request_questions
        ?.are_you_investigator_or_local_principal_investigator ??
        requestDetails?.are_you_investigator_or_local_principal_investigator ??
        false
    ),
    has_co_principal_investigator: Boolean(
      data.ethical_request_questions?.has_co_principal_investigator ??
        requestDetails?.has_co_principal_investigator ??
        false
    ),
    has_project_sponsored: Boolean(
      data.ethical_request_questions?.has_project_sponsored ??
        requestDetails?.has_project_sponsored ??
        false
    ),
    completed_ethics_training: Boolean(
      data.ethical_request_questions?.completed_ethics_training ??
        requestDetails?.completed_ethics_training ??
        false
    ),
    specimen_will_be_shipped_out: Boolean(
      data.ethical_request_questions?.specimen_will_be_shipped_out ??
        requestDetails?.specimen_will_be_shipped_out ??
        false
    ),
    duration_of_research_in_years: Number(
      data.ethical_request_questions?.duration_of_research_in_years ??
        requestDetails?.duration_of_research_in_years ??
        1
    ),
  };
  ...
  async function onSubmit(values: QuestionsSchema) {
    try {
      ...
      // Save the updated data in the Zustand store
      setData({
        ...data,
