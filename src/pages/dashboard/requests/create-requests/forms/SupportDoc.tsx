...
const SupportDoc = ({ handleNext }: Props) => {
  ...
  const createFileSchema = () => {
    ...
    const fileSchemaObject = requiredFiles.reduce((schemas, requirement) => {
      if (
        requirement.id === "requirement6" ||
        requirement.id === "requirement7"
      ) {
        return schemas;
      }

      schemas[requirement.id] = fileSchema
        .nullable()
            ...
            <div className="flex flex-col gap-8 ">
              {requiredFiles.map((requirement) => (
                <CustomFormField
                  ...
                  error={
                    (errors.files as any)?.[requirement.id] as
                      | FieldError
                      | undefined
                  }
...
export function getRequiredFilesBasedOnYes(checkbox: CheckboxGroup) {
  const requiredFiles: { id: string; label: string; name: string }[] =
    Object.keys(requirements)
      .filter(
        (questionKey) =>
          // Filter only questions marked any of the values
          checkbox[questionKey as keyof CheckboxGroup] === "yes" ||
          checkbox[questionKey as keyof CheckboxGroup] ===
            "principal investigator" ||
          checkbox[questionKey as keyof CheckboxGroup] ===
            "local principal investigator" ||
          checkbox[questionKey as keyof CheckboxGroup] === "student"
      )
