...
export default function StatusTracker({
  ...
  return (
    <div
      className={`${
        isMobile ? "" : "md:block hidden max-w-[24.375rem] py-12 fixed"
      }  w-full flex flex-col px-4 mx-auto`}
    >
      <div
        className={`${
          isMobile ? "gap-6" : "gap-8 max-w-[14.7rem] py-16"
        } w-full mx-auto flex flex-col `}
      >
        <div className="flex gap-2 items-center justify-start">
          <h1 className="font-semibold border-b-2 border-[#FFD13A] text-[1.625rem]">
            Status Tracker
          </h1>
          <StatusHelp />
        </div>
        {isMobile && (
          ...
          <div
            onClick={handleFunc}
            className="flex items-center text-[#192C8A] py-2"
          >
